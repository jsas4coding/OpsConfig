...
Major restructuring, security hardening, and code quality release. Scripts moved to `bin/`, shared
library extracted, deprecated Neovim APIs updated, and comprehensive input validation added.
...
- **Install URL changed**: `main/install` → `main/bin/install`
  ```bash
  # New install command
  curl -fsSL https://raw.githubusercontent.com/jsas4coding/OpsConfig/main/bin/install | bash
  ```
- Scripts moved from root to `bin/` directory. If you reference scripts directly, update paths.
- Config file permissions changed from 644 to 600 (more restrictive).

### Upgrade

Existing installations will auto-update via `u` (update alias). The update script handles the path
